...
export class GameStateMachine {
  ...
  handleDiceCheckInteraction(actionData, input) {
    ...
    const roll = Math.floor(Math.random() * 100) + 1;
    const target =
      (skillValue || this.campaign.player.stats[skill] || 50) + (actionData?.modifier || 0);
    const success = roll <= target;
  ...
  checkSkillSuccessEvents(skill, roll, target) {
    ...
    for (const [eventId, event] of Object.entries(this.module.events)) {
      ...
      const skillMatch = trigger.skill.some(
        (s) =>
  ...
  async handleTalk(intent, input = '') {
    ...
    for (const npcId of npcs) {
      ...
      if (!npc) continue;
      if (
        input.toLowerCase().includes(npc.name.toLowerCase()) ||
    ...
    if (matchedNPC) {
      ...
      engine.updateState(decision, {
        trust_delta:
          decision.mood === 'friendly' || decision.mood === 'grateful' || decision.action === 'talk'
            ? 5
            : 0,
      ...
      available_actions: [
        {
          type: 'combat_attack',
          target,
          label: `攻击${this.module.npcs?.[target]?.name || target}`,
        },
  ...
  applyEffect(effect) {
    if (effect.operation === '+' || effect.operation === '-') {
      ...
      this.campaign.global_vars[effect.target] =
        effect.operation === '+' ? current + effect.value : current - effect.value;
