...
export class NPCDecisionEngine extends NPCDecisionEngineContract {
    ...
    _ruleBasedDecision(context) {
        ...
        if (campaign_state.is_combat) {
            ...
            if (campaign_state.is_player_turn && role !== 'Boss') {
                return { action: 'ignore', confidence: 0.7, reasoning: '玩家回合，NPC 等待', mood: 'alert' };
            }
        ...
        if (situation.type === 'player_attack') {
            ...
            if (role === 'enemy') {
                return { action: 'attack', confidence: 0.92, reasoning: '被玩家攻击，反击', mood: 'enraged', target_id: 'player' };
            }
            if (role === 'neutral') {
                return { action: 'flee', confidence: 0.88, reasoning: '无辜被攻击，恐惧逃跑', mood: 'terrified', target_id: 'player' };
            }
            if (role === 'ally') {
        ...
        if (situation.type === 'player_threat') {
            ...
            if (role === 'enemy') {
                return { action: 'attack', confidence: 0.85, reasoning: '威胁激发敌意', mood: 'defiant', target_id: 'player' };
            }
        ...
        if (situation.type === 'player_talk' && situation.player_input) {
            ...
            if (template.secrets && template.secrets.length > 0) {
                const secretKeywords = template.secrets.map(s => s.keyword?.toLowerCase()).filter(Boolean);
    ...
    _attitudeBasedDecision(context) {
        ...
        switch (attitude) {
            case 'friendly':
                return { action: 'talk', confidence: 0.7, reasoning: '友好态度，乐于交流', mood: 'friendly', target_id: 'player', dialogue_topic: 'greeting' };
            case 'hostile':
            case 'hostile_alerted':
                if (role === 'enemy') {
                    return { action: 'attack', confidence: 0.65, reasoning: '敌对态度，准备攻击', mood: 'hostile', target_id: 'player' };
                }
                return { action: 'ignore', confidence: 0.6, reasoning: '敌对但非战斗角色，回避', mood: 'cold', target_id: 'player' };
            case 'afraid':
                return { action: 'flee', confidence: 0.65, reasoning: '恐惧中，保持距离', mood: 'scared', target_id: 'player' };
            case 'neutral':
            default:
                return { action: 'talk', confidence: 0.6, reasoning: '中立态度，保持礼貌', mood: 'neutral', target_id: 'player', dialogue_topic: 'greeting' };
        }
    }
    ...
    async generateDialogue(contextSummary, mood, topic) {
        ...
        if (topic && template.dialogue?.[topic]) {
            ...
            if (unrevealed.length > 0) {
                ...
                actions.push({ action: 'hint', confidence: 0.8, reasoning: 'Secret revealed', metadata: { clue_id: secret.clue_id } });
            }
    ...
    updateState(decision, outcome = {}) {
        ...
        if (outcome.healing_received !== undefined) {
            npc.current_hp = Math.min(npc.current_hp + outcome.healing_received, this.npcTemplate.hp || 10);
        }
