...
function testIndexJsDefaultApiPath() {
  ...
  assert.ok(!indexSrc.includes('localhost:3001'), 'index.js 不应再包含 localhost:3001 mock URL');
...
function testGameControllerDefaults() {
  ...
  assert.ok(
    gcSrc.includes('apiBaseUrl = apiBaseUrl || DEFAULT_API_BASE') ||
      gcSrc.includes('apiBaseUrl || DEFAULT_API_BASE'),
...
function testApiPathConstruction() {
  ...
  // 验证 apiBase 已去除尾部斜杠
  assert.ok(
    gcSrc.includes("apiBaseUrl.replace(/\\/+$/, '')") ||
      gcSrc.includes("apiBaseUrl.replace(/\\/+$/, '')"),
