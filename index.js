...
function showSaveDialog() {
  ...
  popup.querySelector('#ai-gm-save-btn-action').addEventListener('click', async () => {
    try {
      ...
      if (result.success) {
        popup.querySelector('#ai-gm-save-status').textContent =
          `✅ 存档成功 (槽位 ${selectedSlot})`;
        loadSaveSlots(popup);
      }
    } catch (e) {
  ...
  try {
    ...
    slotsContainer.querySelectorAll('.ai-gm-save-slot').forEach((slotEl) => {
      slotEl.addEventListener('click', () => {
        slotsContainer
          .querySelectorAll('.ai-gm-save-slot')
...
function updateCombatUI(result) {
  ...
  if (summary) {
    ...
    document.getElementById('ai-gm-combat-turn').textContent =
      `当前行动: ${summary.current_turn_name || summary.current_turn}`;

    const initiativeList = document.getElementById('ai-gm-combat-initiative');
    initiativeList.innerHTML = summary.enemies
      .map(
        (e) =>
  ...
  const initiativeList = document.getElementById('ai-gm-combat-initiative');
  initiativeList.innerHTML = result.initiative
    .map(
      (entry) =>
...
function updatePanelUI(module) {
  ...
  document.getElementById('ai-gm-scene-title').textContent =
    module.scenes[module.start_scene]?.title || '-';
  document.getElementById('ai-gm-scene-desc').textContent =
    module.scenes[module.start_scene]?.description || '-';

  // Update NPC list
  const npcList = module.scenes[module.start_scene]?.npcs_present || [];
  const npcContainer = document.getElementById('ai-gm-npcs-present');
  npcContainer.innerHTML = npcList
    .map((npcId) => {
      const npc = module.npcs[npcId];
